...
public class BasicKlondike implements KlondikeModel<Card> {
  ...
  // Use a policy object to check move legality
  ...
  public List<Card> createNewDeck() {
    ...
    for (CardSuit s : new CardSuit[] {CardSuit.CLUBS, CardSuit.SPADES, CardSuit.DIAMONDS,
        CardSuit.HEARTS}) {
  ...
  public void startGame(List<Card> deck, boolean shuffle, int numPiles, int numDraw)
    ...
    int index = 0;
    for (int n = 0; n < numPiles; n++) {
      Cascade c = cascades.get(n);
      int count = n + 1;
      for (int k = 0; k < count - 1; k++) {
        c.down.addLast(work.get(index++));
      }
      c.up.addLast(work.get(index++));
    }
  ...
  public void movePile(int srcPile, int numCards, int destPile)
    ...
    if (dst.up.isEmpty()) {
      if (!isKing(movingTop)) {
        throw new IllegalStateException("Destination requires king");
      }
    } else {
      Card destBottom = dst.up.peekLast();
      if (!canPlaceOnCascade(movingTop, destBottom)) {
  ...
  public void moveDraw(int destPile) throws IllegalArgumentException, IllegalStateException {
    ...
    if (dst.up.isEmpty()) {
      if (!isKing(card)) {
        throw new IllegalStateException("Empty destination requires a King");
      }
    } else {
      Card destBottom = dst.up.peekLast();
      if (!canPlaceOnCascade(card, destBottom)) {
        ...
        for (int d = 0; d < n; d++) {
          if (d != s) {
            ...
            if (dst.up.isEmpty()) {
              ...
              if (canPlaceOnCascade(movingTop, dst.up.peekLast())) {
                return false;
              }
            }
          }
  ...
  private void flipIfNeeded(Cascade c) {
    if (c.up.isEmpty() && !c.down.isEmpty()) {
      c.up.addLast(c.down.removeLast());
    }
  }
  ...
  private boolean canPlaceOnCascade(Card movingTop, Card destBottom) {
    return policy.canPlaceOnCascade(movingTop, destBottom);
  }

  private boolean isKing(Card c) {
    return valueOf(c) == 13;
  }
